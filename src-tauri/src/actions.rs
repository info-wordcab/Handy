...
impl ShortcutAction for OpenAppAction {
    ...
    fn stop(&self, app: &AppHandle, binding_id: &str, _shortcut_str: &str) {
        ...
        tauri::async_runtime::spawn(async move {
            ...
            debug!("Starting async app launch task for binding: {}", binding_id);
            ...
            if let Some(samples) = rm.stop_recording(&binding_id) {
                ...
                match tm.transcribe(samples) {
                    Ok(transcription) => {
                        ...
                        if !transcription.is_empty() {
                            ...
                            match alm.find_app(&transcription) {
                                ...
                                None => {
                                    info!("No app found matching: '{}'", transcription);
                                }
                            }
                        }
                        utils::hide_recording_overlay(&ah);
                    ...
                    Err(err) => {
                        debug!("OpenAppAction transcription error: {}", err);
                        utils::hide_recording_overlay(&ah);
                        change_tray_icon(&ah, TrayIconState::Idle);
                    }
                }
        ...
        debug!("OpenAppAction::stop completed in {:?}", stop_time.elapsed());
    }
}
