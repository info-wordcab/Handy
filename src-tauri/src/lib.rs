...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Window is configured to start with label "main".
            ...
            let recording_manager = Arc::new(
                AudioRecordingManager::new(&app.handle()).expect("Failed to initialize recording manager"),
            );
            let model_manager =
                Arc::new(ModelManager::new(&app.handle()).expect("Failed to initialize model manager"));
            let transcription_manager = Arc::new(
                TranscriptionManager::new(&app.handle(), model_manager.clone())
                    .expect("Failed to initialize transcription manager"),
            );
            let history_manager =
                Arc::new(HistoryManager::new(&app.handle()).expect("Failed to initialize history manager"));
            let app_launcher_manager = Arc::new(
                AppLauncherManager::new(&app.handle()).expect("Failed to initialize app launcher manager"),
            );
